...
buble_plot = function(data_enriched,subtitle="",database_slc='') {
  ...
  if(nrow(up) > 0) {
    ...
    up$gene_ratio = up$overlap_int / up$totalpath_int #16599 #Gene ratio is calculated by number of overlapping gene in a pathway divided by overall imput number
  ...
  if(nrow(down) > 0) {
    ...
    down$gene_ratio = down$overlap_int / down$totalpath_int #16599 #Gene ratio is calculated by number of overlapping gene in a pathway divided by overall imput number
...
for(database in dbs){
  ...
  pdf(file = paste("./03_plots/bubble2/enrichment_",database,"_VSmock_bubblesPlot_reginadata.pdf",sep="" ), #Change Filename based on function used
      width = 12,
      height = 7)
